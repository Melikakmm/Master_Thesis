...
def cross_validate(model,transfer_learning: True, folds: list, x_list: list or tuple, y,
    ...
    for i_cv, (train_index, val_index) in enumerate(folds):
        ...
        if transfer_learning == True:
            ...
            for weights in range(10):
                with open('/Users/melikakeshavarz/Desktop/Thesis/Dekany_replication_2/results_g/best_model_g/model.json', 'r') as json_file:
                    loaded_model_json = json_file.read()
                model_ = keras.models.model_from_json(loaded_model_json)
                model_.load_weights(f'/Users/melikakeshavarz/Desktop/Thesis/Dekany_replication_2/results_g/best_model_g/weights_{weights}.h5')
                ...
                cv_train.append(cv_train_output)
                cv_val.append(cv_val_output)
                model_w.append(model_weights)
                scalers_f.append(scalers_folds)
                his.append(histories)

            cv_train_output_en = np.mean(cv_train)
            cv_val_output_en = np.mean(cv_val)



    return cv_train_output_en, cv_val_output_en, model_w, scalers_f, his
