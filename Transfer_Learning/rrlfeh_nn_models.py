...
from tensorflow.keras.layers import Input, Masking, Bidirectional, GRU, Dropout, Dense
from tensorflow.keras.models import Model
from tensorflow.keras.regularizers import L1L2
...
def bigru2p_model(n_timesteps=100, n_channels=2, mask_value=-1,
                   hparams=(16, 16, 'l2', 2e-6, 2e-6, 2e-6, 2e-6, 0.2, 0.2), n_meta=None):
    ...
    x = Bidirectional(GRU(int(dim1), return_sequences=True, kernel_regularizer=kernel_regularizer1,
                           recurrent_regularizer=recurrent_regularizer1))(xs_input, mask=mask)

    if float(dropout1) > 0:
        x = Dropout(rate=float(dropout1))(x)

    x = Bidirectional(GRU(int(dim2), return_sequences=False, kernel_regularizer=kernel_regularizer2,
                           recurrent_regularizer=recurrent_regularizer2))(x)
    ...
    model = Model(inputs=xs_input, outputs=x, name='bigru2p')
    return model
...
available_models = {'bilstm2p': bilstm2p_model, 'bilstm3p': bilstm3p_model,
                    'cnn3_fc': cnn3_fc_model, 'cnn3_gmp_fc_model': cnn3_gmp_fc_model, 'GRU': bigru2p_model }
